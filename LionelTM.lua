...
local Config = {
    ...
    Memory = {
        ...
        RapidFire = 1,
        SpinBot = false,
        LagOpponents = false, -- TÍNH NĂNG MỚI: Làm lag đối thủ
        AutoDodge = false     -- TÍNH NĂNG MỚI: Né chiêu tự động
    }
}
...
-- Helper Function: Kéo thả UI cho cả Icon và Bảng Menu MainFrame
...
-- KÍCH HOẠT DI CHUYỂN BẢNG MENU MAIN FRAME
...
CreateToggle(MemoryTab, "Làm lag đối thủ (Desync Bot)", Config.Memory.LagOpponents, function(v) Config.Memory.LagOpponents = v end)
CreateToggle(MemoryTab, "Né chiêu tự động (Auto-Dodge)", Config.Memory.AutoDodge, function(v) Config.Memory.AutoDodge = v end)
...
RunService.RenderStepped:Connect(function()
    ...
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
        ...
        -- TÍNH NĂNG MỚI 1: LÀM LAG ĐỐI THỦ (Phát Desync / Giật CFrame liên tục để đối thủ bị lệch Aim)
        if Config.Memory.LagOpponents and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            local hrp = LocalPlayer.Character.HumanoidRootPart
            local originalCFrame = hrp.CFrame
            hrp.CFrame = originalCFrame * CFrame.new(math.random(-10, 10), math.random(-5, 5), math.random(-10, 10))
            RunService.Heartbeat:Wait()
            hrp.CFrame = originalCFrame
        end

        -- TÍNH NĂNG MỚI 2: AUTO-DODGE (Tự động né chiêu/đạn khi kẻ địch ở gần và ngắm vào bạn)
        if Config.Memory.AutoDodge and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            local hrp = LocalPlayer.Character.HumanoidRootPart
            for _, plr in pairs(Players:GetPlayers()) do
                if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- Nếu đối thủ áp sát dưới 20 studs, tự động Dịch chuyển giật lùi né đạn
                    if dist < 20 then
